...
export const getPresetFile = (fileName: string): File => {
  const isWindows = $.os.indexOf("Windows") !== -1;
  let presetsPath = "";

  if (isWindows) {
    presetsPath = Folder.startup.fsName + "/Presets";
  } else {
    presetsPath = Folder.startup.parent.parent.parent.fsName + "/Presets";
  }

  return new File(presetsPath + "/Gravitae/" + fileName);
};
